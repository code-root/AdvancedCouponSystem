@extends('layouts.vertical', ['title' => 'Broker Details'])

@section('content')
    @include('layouts.partials.page-title', ['subtitle' => 'Brokers', 'title' => 'Broker Details'])
        ...
        <div class="col-12 mb-3">
            <div class="d-flex justify-content-end gap-2">
                ...
                <a href="{{ route('brokers.create') }}" class="btn btn-primary">
                    <i class="ti ti-plug-connected me-1"></i> Connect This Broker
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-lg-4">
            <div class="card">
                <div class="card-body text-center">
                    ...
                    <h4 class="mb-1">{{ $broker->display_name }}</h4>
                    <p class="text-muted mb-3">{{ $broker->name }}</p>
                    ...
                    <div class="d-flex justify-content-center gap-2">
                        <span class="badge bg-primary-subtle text-primary">{{ $broker->country }}</span>
                        @if($broker->is_active)
                    ...
                    <div class="border border-end-0 border-start-0 border-dashed p-2 mx-n3">
                        <div class="row text-center g-2">
                            <div class="col-6 border-end">
                                <h5 class="mb-1">{{ $broker->commission_rate }}%</h5>
                                <p class="text-muted mb-0 fs-13">Commission</p>
                            </div>
                            <div class="col-6">
                                <h5 class="mb-1">{{ $broker->total_products ?? '1000+' }}</h5>
        ...
        <div class="col-lg-8">
            <div class="card">
                <div class="card-header border-bottom border-dashed">
                    <h4 class="card-title mb-0">Broker Information</h4>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label class="text-muted mb-1">Display Name</label>
                            <p class="fw-semibold mb-0">{{ $broker->display_name }}</p>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="text-muted mb-1">System Name</label>
                            <p class="fw-semibold mb-0">{{ $broker->name }}</p>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="text-muted mb-1">Country</label>
                            <p class="fw-semibold mb-0">{{ $broker->country }}</p>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="text-muted mb-1">Commission Rate</label>
                            <p class="fw-semibold mb-0">{{ $broker->commission_rate }}%</p>
                        </div>
                        <div class="col-12 mb-3">
                            <label class="text-muted mb-1">Description</label>
                            <p class="mb-0">{{ $broker->description }}</p>
                        </div>
                        <div class="col-12 mb-3">
                            <label class="text-muted mb-1">API URL</label>
                            <p class="mb-0"><code>{{ $broker->api_url }}</code></p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="d-flex flex-wrap gap-2">
                        @if($broker->features && is_array($broker->features))
                            @foreach($broker->features as $feature)
